...
//BCC 20191203 END
  ...
  if(context.raw_query && context.raw_count_query){
    const the_count = await model.query(context.raw_count_query,{ replacements: context.raw_query_params, type: Sequelize.QueryTypes.SELECT }).then(count_results =>{
        console.log("count results", count_results);
        return count_results.the_count;
       });
