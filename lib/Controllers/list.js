...
const OP_MAP = {
  "LTE_" : Sequelize.Op.lte,
  "LT_" : Sequelize.Op.lt,
  "GTE_" : Sequelize.Op.gte,
  "GT_" : Sequelize.Op.gt,
}
...
List.prototype.fetch = async function(req, res, context) {
  ...
  var extraSearchCriteria = _.reduce(req.query, function(result, value, key) {
    ...
    if(or_group_flag){
      ...
      if(key.indexOf(DATE_OP_PREFIX) === 0 && req.query[key]) {
        let datesplit = req.query[key].split("_");
        //does it have enough parts?
        if(datesplit.length > 2) {
          let date_op = datesplit[2];
          let core_field = datesplit[3];
          //is it a valid field?
...
//BCC 20191203 END
  ...
  function parseCritObj(critobj, joiner) {
    ...
    if(critobj[Sequelize.Op.or]) {
      raw_where_stack.push(" ( ");
      raw_http_where += " ( ";
      parseCritObj(critobj[Sequelize.Op.or], " OR ");
      raw_http_where += " ) ";
    } else if(critobj[Sequelize.Op.and]) {
      raw_where_stack.push(" ( ");
      raw_http_where += " ( ";
      parseCritObj(critobj[Sequelize.Op.or], " AND ");
      raw_http_where += " ) ";
    } else {
      ...
      if(other_keys && other_keys.length) {
        ...
        if(typeof operand_b_part === "object") {
          if(operand_b_part[Sequelize.Op.Like]) {
            operand_val = operand_b_part[Sequelize.Op.Like];
          }
        } else {
