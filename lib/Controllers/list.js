...
List.prototype.fetch = function(req, res, context) {
  ...
  var extraSearchCriteria = _.reduce(req.query, function(result, value, key) {
    ...
    console.log("req.query, value, key, model, model.rawAttributes, model.rawAttributes[key].type", req.query, value, key, model, model.rawAttributes, model.rawAttributes[key].type )
    ...
    if(or_group_flag){
      ...
      getKeys(req.query).forEach(key =>{
        ...
        let attr_type = model.rawAttributes[key].type;
        console.log("our attribute type", attr_type);

        value_arr.forEach(val => {
          val = val._safeishParse(val, attr_type, Sequelize);
