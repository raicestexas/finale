      ...
      if(key.indexOf(DATE_OP_PREFIX) === 0 && value) {
        ...
        if(datesplit.length > 2) {
          ...
          if (_.has(model.rawAttributes, core_field)) {
            ...
            let constraint = {[our_seq_op] : self._safeishParse(req.query[key], model.rawAttributes[core_field].type, Sequelize) };

            if(result[core_field]) {
              result[core_field] = { [Op.and] : [
                result[core_field],
                constraint
              ]};
            } else {
              result[core_field] = constraint;
            }
          } else {
...
//BCC 20191203 END
  ...
  function parseCritObj(critobj, joiner) {
    ...
    if(critobj[Op.or]) {
      ...
      let other_keys = Object.keys(critobj);
